...
void _bluetooth_transmit_csi(const char* csi_data) {
    #ifdef CONFIG_SEND_CSI_TO_BLUETOOTH
    // Find an active SPP connection and send data
    esp_spp_write(SPP_HANDLE, strlen(csi_data), (uint8_t*)csi_data);
    #endif
}
...
#ifdef CONFIG_SEND_CSI_TO_BLUETOOTH
    _bluetooth_transmit_csi(ss.str().c_str());
#endif
